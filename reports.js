...
window.exportHTMLReport = async function() {
    ...
    if (!profileLogs.length) {
        if(typeof showVaultStatus === 'function') showVaultStatus(`No logs found for ${activeProfile}.`, "var(--danger-color)");
        return;
    }

    // 3. Prepare Refill Requirements
    ...
    profileLogs.sort((a,b) => new Date(b.dateTaken) - new Date(a.dateTaken)).forEach(l => {
        // Use logicalDate if available, fallback to dateTaken locale string
